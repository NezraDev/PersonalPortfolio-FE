...
function ProjectsMobile() {
  ...
  return (
    <div
      className="relative z-10 py-8 px-2"
      id="projects"
      style={style}
      ref={sectionRef}
    >
      <div className="max-w-7xl mx-auto">
        <div className="relative top-8 text-center mb-12 anton">
          <h1 className="text-3xl font-bold text-yellow-500 mb-4 drop-shadow-lg">
            <span className="text-white">Here are some of my</span>{" "}
            Collaborative Projects
          </h1>
          <p className="text-lg text-yellow-500 drop-shadow">
        ...
        <div
          className={`inter grid grid-cols-1 gap-4 transition-all duration-300 ${
            isModalOpen ? "blur-sm" : ""
          }`}
        >
          {projects.map((project) => (
            <div
              key={project.id}
              className="project-card group relative bg-black bg-opacity-40 backdrop-blur-md rounded-2xl shadow-xl overflow-hidden cursor-pointer min-h-[320px] flex flex-col border border-yellow-500 border-opacity-30"
              onClick={() => openModal(project)}
            >
              <div className="h-32 bg-gradient-to-br from-yellow-500 to-black relative overflow-hidden flex-shrink-0">
                {!imageErrors[project.id] ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-yellow-500 to-black">
                    <div className="text-yellow-500 text-center p-4">
                      ...
                      <p className="text-xs text-yellow-500">Project Image</p>
                    </div>
                  </div>
                )}
                <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="text-yellow-500 text-center p-3">
                    <h3 className="text-sm font-semibold mb-1 drop-shadow-lg line-clamp-2">
                      {project.title}
                    </h3>
              ...
              <div className="flex-1 flex flex-col justify-between p-4 text-yellow-500">
                <div className="min-h-[60px] flex flex-col justify-start mb-3">
                  <div className="flex flex-wrap gap-1">
                    {project.techStack.slice(0, 2).map((tech, index) => (
                      <span
                        key={index}
                        className="px-2 py-1 bg-black bg-opacity-70 backdrop-blur-sm rounded-full text-xs font-medium border border-yellow-500 border-opacity-50 text-yellow-500"
                      >
                        {tech}
                      </span>
                    ))}
                    {project.techStack.length > 2 && (
                      <span className="px-2 py-1 bg-black bg-opacity-70 backdrop-blur-sm rounded-full text-xs font-medium border border-yellow-500 border-opacity-50 text-yellow-500">
                ...
                <div className="flex-1 mb-3">
                  <p className="text-yellow-300 text-opacity-90 text-justify text-xs leading-relaxed line-clamp-3">
                    {project.description}
                  </p>
                </div>

                <div className="flex justify-between items-center pt-3 border-t border-yellow-500 border-opacity-30">
                  <div className="text-yellow-500 font-semibold text-xs drop-shadow">
                    <div className="text-yellow-400">{project.date}</div>
                  </div>
                  <div className="w-8 h-8 bg-black bg-opacity-70 rounded-full flex items-center justify-center transform group-hover:scale-110 transition-transform duration-300 border border-yellow-500">
                    <svg
        ...
        {isModalOpen && selectedProject && (
          <div className="fixed inset-0 flex items-center justify-center p-2 z-50">
            <div
              className="absolute inset-0 bg-black bg-opacity-70 backdrop-blur-sm transition-all duration-300"
              onClick={closeModal}
              aria-hidden="true"
            />

            <div className="relative z-10 bg-black bg-opacity-95 backdrop-blur-lg rounded-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-yellow-600 border-opacity-50">
              <div className="p-4">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <button
                    onClick={closeModal}
                    className="text-yellow-400 hover:text-yellow-300 transition-colors duration-200 bg-black bg-opacity-70 rounded-full p-1 backdrop-blur-sm border border-yellow-600 border-opacity-50"
                  >
                    <svg
                      className="w-5 h-5"
                      fill="none"
                ...
                <h2 className="text-lg font-bold text-yellow-300 mb-3 drop-shadow-lg text-center">
                  {selectedProject.title}
                </h2>

                <div className="h-40 bg-gradient-to-br from-yellow-500 to-black rounded-lg mb-4 relative overflow-hidden">
                  {!imageErrors[selectedProject.id] ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-yellow-700 to-black">
                      <div className="text-yellow-500 text-center p-3">
                        ...
                        <p className="text-yellow-400 text-sm">Project Image</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="mb-4">
                  <h3 className="text-lg font-semibold text-yellow-400 mb-2 drop-shadow">
                    Tech Stack
                  </h3>
                ...
                <div className="mb-4">
                  ...
                  <p className="text-yellow-300 text-justify leading-relaxed text-sm bg-black bg-opacity-50 p-3 rounded-xl border border-yellow-600 border-opacity-50 backdrop-blur-sm">
                    {selectedProject.description}
                  </p>
                </div>

                <div className="pt-3 border-t border-yellow-600 border-opacity-50">
